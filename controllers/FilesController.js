...
import { isValidUser, getIdAndKey } from '../utils/db';
...
class FilesController {
  ...
  static async putPublish(request, response) {
      ...
      const currentUser = await dbClient.users.findOne({ _id: ObjectId(userId) });
      if (!currentUser) return response.status(401).send({ error: 'Unauthorized' });
  
      const fileId = request.params.id || '';
  
      let file = await dbClient.files.findOne({ _id: ObjectId(fileId), userId: currentUser._id });
      if (!file) return response.status(404).send({ error: 'Not found' });
  
      await dbClient.files.updateOne({ _id: ObjectId(fileId) }, { $set: { isPublic: true } });
      file = await dbClient.files.findOne({ _id: ObjectId(fileId), userId: currentUser._id });
